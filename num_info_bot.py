...
API_URL = "https://learnonix-api.onrender.com/numbers"
FORCE_CHANNELS = ["@swapipy", "@adipython", "@brontzpy", "@ILLUMINATIHUBS", "@roxug","@team_senpieee","@pradumpyt"]  # multiple channels
REQUIRED_CREDITS = 1
# ==========================================
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =============== INPUT HANDLERS ===============
        ...
        try:
            resp = requests.get(f"{API_URL}/{number}", timeout=15).json()
            ...
            save_user(user)
            for i, entry in enumerate(resp, 1):
                result = (
                    f"💠 ━━━ Result {i} ━━━ 💠\n"
                    f"📱 Mobile: <b>{entry.get('mobile','-')}</b>\n"
                    f"👤 Name: <b>{entry.get('name','-')}</b>\n"
                    f"👨 Father: <b>{entry.get('father_name','-')}</b>\n"
                    f"🏠 Address: <b>{entry.get('address','-').replace('!',' ')}</b>\n"
                    f"📞 Alt: <b>{entry.get('alt_mobile','-')}</b>\n"
                    f"🌐 Circle: <b>{entry.get('circle','-')}</b>\n"
                    f"🆔 ID: <b>{entry.get('id_number','-')}</b>\n"
                    f"✉️ Email: <b>{entry.get('email','-')}</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━"
                )
